...
folderRaw  <- paste0(folderRaw, "/live_data")
print(folderRaw)
#마지막에 슬래시를 붙이지 않는다.
#마지막에 슬래시를 넣는다.
...
folderWrite  <- paste0(folderWrite, "/live_data")
print(folderWrite)
...
fileName <- paste0(folderPopu, "/common_file/11_2019년_성연령별인구.txt")
print(fileName)
...
st_write(popuShp, paste0(folderWrite,"/live_popu_100.shp"), driver="ESRI Shapefile")
